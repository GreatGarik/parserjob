...
for link in links:
    ...
    article = soup.find('div', class_="goods_artikul").text.split(':')[1].strip().replace('*', '-') if soup.find('div', class_="goods_artikul") and len(soup.find('div', class_="goods_artikul").text.split(':')) > 1 else f'Нет артикула {name}'.replace('/', '-')
    ...
    if not os.path.exists(f'Aurora_ru/{article}'):
        images = [i['href'] for i in soup.find_all('a', class_="goods_gallery")]
        os.mkdir(f'Aurora_ru/{article}')
        for image in images:
            try:
                response = requests.get(url=f'https://aurora.ru{image}', stream=True)
